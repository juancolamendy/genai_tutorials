...
A Model Context Protocol (MCP) server that provides Google Analytics 4 data access through various tools and functions.
...
```json
{
    "mcpServers": {
        "ga_mcp": {
            ...
            "env": {
                "GOOGLE_APPLICATION_CREDENTIALS": "<path>/ga_mcp/api_svc_acct.json",
            }
        }
    }
}
...
```json
{
  "mcpServers": {
    "ga_mcp": {
        ...
        "args": [
            "--directory",
            "/Users/jcolamendy/ai_ml/mcp_servers/ga_mcp",
            "run",
            "main.py"
        ],
        "env": {
            "GOOGLE_APPLICATION_CREDENTIALS": "/Users/jcolamendy/ai_ml/mcp_servers/ga_mcp/api-calls-project-google_analytics_api_svc_acct.json"
        }
    }
  }
}
...
```
Your task is to use the ga_mcp to analyze the property 426342331 in Google Analytics.
Follow the instructions:
- analyze the last month traffic to the property
- provide a list of the top pages visited
- provide insights about the traffic in the last month
- provide strategies to improve the traffic
```
