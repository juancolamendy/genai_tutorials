...
---
    uv run main.py
    uv run main.py --verbose          # show per-case reasoning
    uv run main.py --threshold 0.9    # stricter pass bar
    uv run main.py --model gemini-2.0-pro  # higher quality judge
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="LLM-as-Judge for Google ADK agents")
    p.add_argument("--model",     default="gemini-2.5-flash",
                   help="Gemini model to use as judge (default: gemini-2.5-flash)")
...
tracer = trace.get_tracer("llm_judge_adk")
...
def build_judge_agent(model: str) -> LlmAgent:
    """Create a stateless LlmAgent used as the evaluation judge."""
    return LlmAgent(
        name        = "llm_judge",
        model       = model,
...
class LLMJudge:
    """
    LLM-as-Judge evaluator for Google ADK agents.
    ...
    def __init__(self, model: str = "gemini-2.5-flash", threshold: float = 0.7) -> None:
        self.threshold  = threshold
        self._agent     = build_judge_agent(model)
        self._session   = InMemorySessionService()
        ...
        self._session.create_session_sync(
...
async def main() -> None:
    print("=" * 60)
    print("  LLM-as-Judge — Google ADK  |  Ticket Processing Agent")
    print("=" * 60)
